...
router.route("/rsvp/:id").post(async (req, res) => {
  try {
    const inserted = await postData.rsvpPost(req.params.id, req.body.user);
    res.json(inserted);
  } catch (e) {
    res.json({ error: e });
  }
...
router.route("/categories").get(async (req, res) => {
  ...
  for (let post of groups) {
    if (!categories.includes(post.name)) categories.push(post.name);
  }
  res.json(categories);
});
